...
def all_zones() -> tuple[str, ...]:
    ...
    with _lock:
        if _cached is not None:
            return _cached
        # Through the facade, not local_db: on a Postgres deploy a direct
        # local_db import reads the frozen SQLite snapshot instead of the
        # live warehouse, and the zone filter silently offers stale zones.
