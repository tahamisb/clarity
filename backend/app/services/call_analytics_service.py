...
def _summary_sync() -> dict:
    ...
    # Tie-broken by label: Python's sort is stable, so equal volumes keep
    # dict insertion order, which follows an unordered GROUP BY and
    # therefore differs between storage engines. Without this the top
    # call reason changes depending on which warehouse is behind it.
    reason_dist = sorted(reasons.values(), key=lambda r: (-r["volume"], r["reason"]))[:15]
